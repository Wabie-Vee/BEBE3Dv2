...
func update(delta):
	if GameManager.player_state == GameManager.PlayerState.FREE:
		...
		player.velocity.x = lerp(
			player.velocity.x, direction.x * speed, player.slide_factor * delta
		)
		player.velocity.z = lerp(
			player.velocity.z, direction.z * speed, player.slide_factor * delta
		)
		...
		if (
			Input.is_action_just_pressed("key_jump")
